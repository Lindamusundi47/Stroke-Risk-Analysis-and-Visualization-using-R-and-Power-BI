---
title: "Stroke Analysis-3"
author: "Linda Musundi"
date: "`r Sys.Date()`"
...
```{r Removing excel helper rows and cleaning column names }
library(dplyr)
...
```{r converting bmi to numeric}
stroke_df <- stroke_df %>%
  mutate(bmi_num = as.numeric(bmi))
summary(stroke_df$bmi_num)
```

```{r converting categorical variales to factors}
stroke_df <- stroke_df %>%
  mutate(
...
```{r visualization of the EDAs Above}
...
write.csv(stroke_df, 
          ...
          row.names = FALSE)
```
```{r exporting the data to be used in Power BI}
